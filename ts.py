...
The column of the data correspond to the
  - year
  - month
  - day
  - hour
...
'''Heroku paths'''
q1_city_year_month_hour_model_txt = open('/app/questions/Assignment_5_TS/ts_Q1_month_hour_model.txt', 'rb+')

'''Digital Ocean paths'''
#q1_city_year_month_hour_model_txt = open('/home/vagrant/miniprojects/questions/Assignment_5_TS/ts_Q1_month_hour_model.txt', 'rb+')

'''Loading models from dilled txt files'''
q1_city_year_month_hour_model = dill.load(q1_city_year_month_hour_model_txt)


'''Q1 MODEL - Initializing class containing Month Hour Model'''
...
class MonthHourModel(TimeSeriesRecordMixin, Question):
  ...
  The simplest (and perhaps most robust) is to have a set of indicator variables for each month. That is to say, make the assumption that the temperature at any given time is a function of only the month of the year, and the hour of the day. **Question**: should month be a continuous or categorical variable?  Predict the temperature value.  As you can imagine, the temperature values will be stripped out in the actual text records that are passed.
  """
    @list_or_dict
    def solution(self, txt):
        txt = txt.split()
        city_year_month_hour_model_class = q1_tsm_MH()
        return city_year_month_hour_model_class.transform(txt)
